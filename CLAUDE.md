...
- Requires a `.env` file; key variables: `PORT` (default 7000), `MONGODB_URL`, `ACCESS_TOKEN_SECRET`, `PAYSTACK_SECRET_KEY`, `NODE_ENV`, and (Phase 6 bot) the LLM provider config — provider-configurable: `BOT_PROVIDER` (`openai` | `anthropic`; optional, auto-detected from whichever key is set, OpenAI preferred). For OpenAI: `OPENAI_API_KEY`, `OPENAI_MODEL` (default `gpt-4o-mini`). For Anthropic: `ANTHROPIC_API_KEY`, `BOT_MODEL` (default `claude-haiku-4-5`). No key for the active provider ⇒ rules-only keyword fallback (bot never hard-fails).
