    ...
    try {
      ...
      if (post.billingType === BILLING_TYPE.PAY_FROM_SUBSCRIPTION) {
        ...
        if (post.items.length > subscriptionPlanMonthlyLimits) {
          return BaseService.sendFailedResponse({
            error:
              "You selected items has exceeded your currently subscription limit. Consider upgrading or reducing your items",
          });
        }
        ...
        if (totalPrice > wallet.balance) {
          return BaseService.sendFailedResponse({
            error:
              "Insufficient balance in your wallet. Please try funding your account to continue",
          });
        }
