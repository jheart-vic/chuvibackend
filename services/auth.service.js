...
`;
  ...
  async createIntakeUser(req, res) {
    try {
      ...
      const userExists = await IntakeUserModel.findOne({ email: post.email });
      if (userExists) {
        return BaseService.sendFailedResponse({
          error: "User exists. Please login",
        });
      ...
      // Create the user
      const newUser = new IntakeUserModel({
        email: post.email,
        password: post.password,
        fullName: post.fullName,
        phoneNumber: post.phoneNumber,
  ...
  async googleIntakeUserSignup(req, res) {
    try {
      ...
      // Check if user exists
      const userWithSub = await IntakeUserModel.findOne({
        $or: [{ googleId }, { email }],
      });
...
if (userWithSub) {
  ...
  return BaseService.sendSuccessResponse({
    ...
    refreshToken,
      ...
      const newUser = new IntakeUserModel(userObject);
      await newUser.save();
  ...
  async appleIntakeUserSignup(req, res) {
    try {
      ...
      // 7. Check if the user already exists in the database
      const userWithAppleId = await IntakeUserModel.findOne({
        $or: [{ appleId }, { email }],
      });
  ...
  async verifyIntakeUserOTP(req) {
    try {
      ...
      const userExists = await IntakeUserModel.findOne({ email }).select(
        "otp otpExpiresAt"
      );
      if (empty(userExists)) {
        return BaseService.sendFailedResponse({
  ...
  async resendIntakeUserOtp(req, res) {
    try {
      ...
      const userExists = await IntakeUserModel.findOne({ email }).select("+password");
  ...
  async forgotIntakeUserPassword(req, res) {
    try {
      ...
      const userExists = await IntakeUserModel.findOne({ email });
      if (!userExists) {
        return BaseService.sendFailedResponse({ error: "User not found" });
      }
      // Generate OTP
...
`;
  ...
  async verifyIntakeUserResetPasswordOtp(req, res) {
    try {
      ...
      const user = await IntakeUserModel.findOne({ email }).select(
        "+resetPasswordOtp +resetPasswordOtpExpiresAt"
      );
  ...
  async resetIntakeUserPassword(req, res) {
    try {
      ...
      const user = await IntakeUserModel.findById(decoded.userId).select("+password");
  ...
  async refreshIntakeUserToken(req, res) {
    try {
      const refreshToken = req.headers["x-refresh-token"]; // better than Authorization
  ...
  async createQCUser(req, res) {
    try {
      ...
      const userExists = await QCUserModel.findOne({ email: post.email });
      if (userExists) {
        return BaseService.sendFailedResponse({
          error: "User exists. Please login",
        });
  ...
  async googleQCUserSignup(req, res) {
    try {
      ...
      // Check if user exists
      const userWithSub = await QCUserModel.findOne({
        $or: [{ googleId }, { email }],
      });
...
if (userWithSub) {
  ...
  return BaseService.sendSuccessResponse({
    ...
    refreshToken,
      ...
      const newUser = new QCUserModel(userObject);
      await newUser.save();
  ...
  async appleQCUserSignup(req, res) {
    try {
      ...
      // 7. Check if the user already exists in the database
      const userWithAppleId = await QCUserModel.findOne({
        $or: [{ appleId }, { email }],
      });
  ...
  async verifyQCUserOTP(req) {
    try {
      ...
      const userExists = await QCUserModel.findOne({ email }).select(
        "otp otpExpiresAt"
      );
      if (empty(userExists)) {
        return BaseService.sendFailedResponse({
  ...
  async resendQCUserOtp(req, res) {
    try {
      ...
      const userExists = await QCUserModel.findOne({ email }).select("+password");
  ...
  async forgotQCUserPassword(req, res) {
    try {
      ...
      const userExists = await QCUserModel.findOne({ email });
      if (!userExists) {
        return BaseService.sendFailedResponse({ error: "User not found" });
      }
      // Generate OTP
...
`;
  ...
  async verifyQCUserResetPasswordOtp(req, res) {
    try {
      ...
      const user = await QCUserModel.findOne({ email }).select(
        "+resetPasswordOtp +resetPasswordOtpExpiresAt"
      );
  ...
  async resetQCUserPassword(req, res) {
    try {
      ...
      const user = await QcUserModel.findById(decoded.userId).select("+password");
  ...
  async refreshQCUserToken(req, res) {
    try {
      const refreshToken = req.headers["x-refresh-token"]; // better than Authorization
