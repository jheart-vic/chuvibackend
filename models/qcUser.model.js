...
qcUserSchema.pre("save", async function (next) {
  try {
...
qcUserSchema.pre("save", function (next) {
...
qcUserSchema.methods.comparePassword = async function (password) {
  const user = this;
...
qcUserSchema.methods.generateAccessToken = async function (secretToken, expiresIn="1w") {
...
qcUserSchema.methods.generateRefreshToken = async function (secretToken) {
  const token = jwt.sign(
    {
      id: this._id,
      userType: this.userType,
...
const QcUserModel = mongoose.model("QcUser", qcUserSchema);
module.exports = QcUserModel;
