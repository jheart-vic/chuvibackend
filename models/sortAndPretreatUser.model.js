...
const sortAndPretreatSchema = new mongoose.Schema(
  {
    ...
    userType: {
      type: String,
      default: ROLE.SORT_AND_PRETREAT,
    },
...
sortAndPretreatSchema.pre("save", async function (next) {
  try {
...
sortAndPretreatSchema.pre("save", function (next) {
...
sortAndPretreatSchema.methods.comparePassword = async function (password) {
  const user = this;
...
sortAndPretreatSchema.methods.generateAccessToken = async function (secretToken, expiresIn="1w") {
...
sortAndPretreatSchema.methods.generateRefreshToken = async function (secretToken) {
  const token = jwt.sign(
    {
      id: this._id,
      userType: this.userType,
...
const sortAndPretreatModel = mongoose.model("sortAndPretreat", sortAndPretreatSchema);
module.exports = sortAndPretreatModel;
